...
export async function GetQuoteForUser(req, res) {
  JWT.verify(req.token, process.env.SecretJwtKey, async (error, authData) => {
    if (authData) {
      ...
      messageData.push({
        role: "system",
        content: `Generate a hyper personalized daily prompt in the form of a question that the user can answer within a journal(Not a quote by someone) of max 200 characters for user ${user.name} considering the time of day ${timeOfDay}.

Consider the user's last journal entry which is ${journalText}. If the journal entry is empty, disregard. 
Also consider the user's goals which is ${goalText}. If the goals are not provided, disregard. 
